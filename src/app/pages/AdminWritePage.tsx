...
function Workspace({userId,postId}:{userId:number;postId?:string}){
 ...
 <fieldset disabled={!d.ready||!!d.recovery} className="article-writing-fields"><label>标题<input value={d.form.title} maxLength={255} onChange={e=>change('title',e.target.value)} placeholder="未命名草稿"/></label>
 {d.ready&&<ArticleEditor value={d.form.content} onChange={value=>change('content',value)} existing={!!postId||!!d.draft?.payload?.content} uploadImage={upload} onError={setUploadError} onBusy={()=>{}}/>}
 ...
 <div className="article-save-bar"><button disabled={!d.ready||d.busy||d.blocked} onClick={()=>void d.save()}>保存草稿</button><button disabled={!d.ready||d.busy||d.blocked||uploads>0} onClick={()=>void d.action('publish')}>{d.draft?.post_status==='published'?'更新发布':'发布文章'}</button>{d.draft?.post_status==='published'&&<button disabled={d.busy||d.blocked} onClick={()=>{if(confirm('撤回后读者将无法访问这篇文章，确定撤回？'))void d.action('withdraw');}}>撤回为草稿</button>}{d.draft?.post_status==='published'&&<Link to={`/article/${d.draft.post_id}`}>查看文章</Link>}</div>
 </div>;
}
