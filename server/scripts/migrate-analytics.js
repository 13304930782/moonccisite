...
async function run() {
  ...
  const filename = '202609140001_analytics.sql';
  ...
  try {
    ...
    if (rows[0] && rows[0].checksum !== checksum) throw new Error('Analytics migration checksum mismatch; stopped without editing history');
    if (!rows.length) {
      await connection.query(sql);
      await connection.query('INSERT INTO schema_migrations(filename,checksum) VALUES (?,?)', [filename, checksum]);
    }
    for (const table of ['analytics_meta','analytics_visits','analytics_daily','analytics_article_totals']) await connection.query(`SELECT * FROM ${table} LIMIT 0`);
    console.log('[analytics-migrate] new tables verified; historical migrations untouched');
  } finally { await connection.query("SELECT RELEASE_LOCK('mooncci_schema_migrations')"); await connection.end(); }
}
run().catch(() => { console.error('[analytics-migrate] migration/preflight failed; inspect database prerequisites before retrying'); process.exitCode = 1; });
