...
async function run() {
  ...
  const filename = '202609110001_article_drafts.sql';
  ...
  try {
    ...
    if (rows[0] && rows[0].checksum !== checksum) throw new Error('Draft migration checksum mismatch; stopped without editing history');
    if (!rows.length) {
      const [columns]=await connection.query("SHOW COLUMNS FROM posts LIKE 'version'");
      if(columns.length && !/^int/.test(columns[0].Type))throw new Error('Unexpected version type');
      await connection.query(columns.length?sql.replace(/^ALTER TABLE posts ADD COLUMN version INT NOT NULL DEFAULT 1;\s*/, ''):sql);
      await connection.query('INSERT INTO schema_migrations(filename,checksum) VALUES (?,?)', [filename, checksum]);
    }
    for (const [table, columns] of Object.entries({posts:'id,version',article_drafts:'id,author_id,post_id,base_version,version,payload,dirty,published_version,updated_at'})) {
      await connection.query(`SELECT ${columns} FROM ${table} LIMIT 0`);
    }
    console.log('[draft-migrate] new tables verified; historical migrations untouched');
  } finally { await connection.query("SELECT RELEASE_LOCK('mooncci_schema_migrations')"); await connection.end(); }
}
run().catch(() => { console.error('[draft-migrate] migration/preflight failed; inspect database prerequisites before retrying'); process.exitCode = 1; });
