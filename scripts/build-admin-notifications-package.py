"""Build the account notification deployment bundle with explicit LF metadata on Windows."""
...
NAME = 'mooncci-admin-notifications-20260908'
SOURCES = ['src/app/pages/AdminUsersPage.tsx', 'src/app/pages/AdminCommentsPage.tsx', 'server/src/routes/admin.js', 'server/src/lib/mailer.js', 'server/src/lib/adminNotification.js']
...
def main():
    ...
    for name in ['deploy-admin-notifications.sh', 'SOURCE-FILES.txt', 'SHA256SUMS']:
        assert b'\r\n' not in entries[name]
    ...
    (ROOT / '.cache/admin-notifications-package.json').write_bytes(json.dumps(result, indent=2).encode('utf-8'))
