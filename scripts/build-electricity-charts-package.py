"""Build the electricity chart deployment bundle with explicit LF metadata on Windows."""
...
NAME = 'mooncci-electricity-charts-20260908'
...
def main():
    ...
    entries['deploy-electricity-charts.sh'] = (ROOT / 'scripts/deploy-electricity-charts.sh').read_bytes().replace(b'\r\n', b'\n')
    entries['SOURCE-FILES.txt'] = ('\n'.join(SOURCES) + '\n').encode('utf-8')
    entries['SHA256SUMS'] = ''.join(
        f'{hashlib.sha256(data).hexdigest()}  {name}\n'
        for name, data in sorted(entries.items())
    ).encode('utf-8')
    for name in ['deploy-electricity-charts.sh', 'SOURCE-FILES.txt', 'SHA256SUMS']:
        assert b'\r\n' not in entries[name]
    ...
    (ROOT / '.cache/electricity-charts-package.json').write_bytes(json.dumps(result, indent=2).encode('utf-8'))
