...
NAME = 'mooncci-electricity-access-layout-20260909'
...
def main():
    ...
    entries['deploy-electricity-access-layout.sh'] = (ROOT / 'scripts/deploy-electricity-access-layout.sh').read_bytes().replace(b'\r\n', b'\n')
    entries['SOURCE-FILES.txt'] = ('\n'.join(SOURCES) + '\n').encode('utf-8')
    entries['SHA256SUMS'] = ''.join(
        f'{hashlib.sha256(data).hexdigest()}  {name}\n'
        for name, data in sorted(entries.items())
    ).encode('utf-8')
    for name in ['deploy-electricity-access-layout.sh', 'SOURCE-FILES.txt', 'SHA256SUMS']:
        assert b'\r\n' not in entries[name]
    ...
    (ROOT / '.cache/electricity-access-layout-package.json').write_bytes(json.dumps(result, indent=2).encode('utf-8'))
