...
migration=server/database/migrations/202609050002_create_electricity_reports.sql
...
backup="/www/backup/mooncci-electricity-rss-$(date +%Y%m%d-%H%M%S)"
...
rsync -a --chmod=D755,F644 "$bundle/dist/" "$site_root/"
su -s /bin/bash mooncci -c 'pm2 save'
echo '电量 RSS 增量部署完成。请确认 Nginx 私密路径配置已校验并重载，再登录站长账号创建订阅链接。'
