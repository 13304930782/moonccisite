...
flock -w 120 9
backup=$(mktemp -d /www/backup/mooncci-dependency-health.XXXXXX)
chmod 700 "$backup"
changed=0
...
while IFS= read -r file;do
 case "$file" in src/index.js|src/lib/dependencyHealth.js|src/routes/dependencyHealth.js) ;; *) echo 'Unexpected file manifest';exit 1;; esac
 test -s "server/$file"
...
pmrestart
health
# Verify the new route is mounted without making external requests.
curl -sS --max-time 5 -o "$backup/route.json" -w '%{http_code}' http://127.0.0.1:3001/api/health/dependencies/unknown > "$backup/route-status"
test "$(cat "$backup/route-status")" = 404
node -e 'if(JSON.parse(require("fs").readFileSync(process.argv[1])).ok!==false)process.exit(1)' "$backup/route.json"
...
install -m 644 dist/index.html "$web/.dependency-health-index.tmp"
mv -f "$web/.dependency-health-index.tmp" "$web/index.html"
echo 'Dependency health deployed. No migrations were executed.'
