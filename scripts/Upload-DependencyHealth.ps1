...
$release = Get-Content -LiteralPath (Join-Path $projectRoot '.cache/dependency-health-release.json') -Raw | ConvertFrom-Json
$packagePath = $release.package
$packageName = Split-Path $packagePath -Leaf
if ($packageName -notmatch '^mooncci-dependency-health-[0-9a-f]{12}\.tar\.gz$') { throw 'Invalid release filename' }
