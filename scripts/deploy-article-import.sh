...
flock -w 120 9
backup=$(mktemp -d /www/backup/mooncci-article-import.XXXXXX)
chmod 700 "$backup"
changed=0
...
finish(){
 ...
 if [ "$rc" -ne 0 ] && [ "$changed" = 1 ];then
   ...
   pmrestart && health
   echo "Failed; restored original files. New unused dependencies may remain."
 fi
 echo "Backup: $backup"
...
(async()=>{try{await db.query('SELECT id,version,payload FROM article_drafts LIMIT 0');await db.query('SELECT version FROM posts LIMIT 0');await db.query('SELECT filename,status FROM media_assets LIMIT 0');}finally{await db.end();}})().catch(e=>{console.error(e.message);process.exitCode=1;});
NODE
: > "$backup/existing-files"
while IFS= read -r file;do
 case "$file" in src/routes/articleDrafts.js|src/lib/articleImport.js|src/lib/articleImportFetch.js|src/lib/articleImportConvert.js|package.json|package-lock.json) ;; *) echo 'Unexpected file manifest';exit 1;; esac
 test -s "server/$file"
 if [ -e "$live/$file" ];then printf '%s\n' "$file" >> "$backup/existing-files";fi
 if [[ "$file" == *.js ]];then node --check "server/$file";fi
done < BACKEND_FILES
for dep in turndown @mixmark-io/domino;do if [ -d "$live/node_modules/$dep" ];then printf '%s\n' "node_modules/$dep" >> "$backup/existing-files";fi;done
...
for dep in turndown @mixmark-io/domino;do
 mkdir -p "$live/node_modules/$(dirname "$dep")"
 cp -a "dependencies/$dep" "$live/node_modules/$(dirname "$dep")/"
 chown -R mooncci:mooncci "$live/node_modules/$dep"
done
node - "$live" <<'NODE'
const path=require('path');const live=process.argv[2];require(path.join(live,'src/lib/articleImport'));console.log('Import dependencies load successfully');
NODE
pmrestart
health
...
install -m 644 dist/index.html "$web/.article-import-index.tmp"
mv -f "$web/.article-import-index.tmp" "$web/index.html"
echo 'Article import and review deployed. No migrations were executed.'
