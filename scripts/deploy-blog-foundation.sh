...
flock -w 120 9
backup=$(mktemp -d /www/backup/mooncci-blog-foundation.XXXXXX)
chmod 700 "$backup"
changed=0
...
while IFS= read -r file;do
 case "$file" in src/routes/posts.js|src/lib/articleDiscovery.js|src/routes/settings.js|src/lib/blogPages.js|src/index.js|src/routes/seo.js|src/lib/seo.js) ;; *) echo 'Unexpected file manifest';exit 1;; esac
 test -s "server/$file"
...
install -m 644 dist/index.html "$web/.blog-foundation-index.tmp"
mv -f "$web/.blog-foundation-index.tmp" "$web/index.html"
echo 'Blog foundation deployed. No migrations were executed.'
