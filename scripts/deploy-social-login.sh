...
for file in "${backend_files[@]}"; do
  [[ "$file" =~ ^(src/(index.js|routes/(auth-cookie|socialLogin).js|lib/(authSession|socialConfig|socialProviders).js)|database/migrations/202609100001_social_login.sql)$ ]]
  test -s "server/$file"
...
log '检查并创建第三方登录所需新表，现有服务继续运行……'
node server/scripts/migrate-social-login.js "$live"
log '更新登录 API，保留 Google CF 代理与环境配置……'
server_changed=1
for file in "${backend_files[@]}"; do
