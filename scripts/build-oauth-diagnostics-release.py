...
entries['deploy.sh'] = (root / 'scripts/deploy-oauth-diagnostics.sh').read_bytes().replace(b'\r\n', b'\n')
files = ['src/routes/socialLogin.js', 'src/lib/socialProviders.js']
for name in files:
...
result = {'revision': revision, 'package': str(output), 'bytes': output.stat().st_size, 'sha256': digest, 'scope': 'oauth-diagnostics-no-migration'}
