...
await page.getByRole('button',{name:'预览',exact:true}).click();await page.getByRole('heading',{name:'Automatic draft',exact:true}).waitFor();await page.getByRole('button',{name:'返回编辑',exact:true}).first().click();
