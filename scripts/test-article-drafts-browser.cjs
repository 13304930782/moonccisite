...
await page.getByRole('button',{name:'预览',exact:true}).click();await page.frameLocator('iframe[title="文章设备预览"]').getByRole('heading',{name:'Automatic draft',exact:true}).waitFor();await page.getByRole('button',{name:'返回编辑',exact:true}).first().click();
await page.getByRole('textbox',{name:'Markdown 正文'}).evaluate(el=>el.setSelectionRange(0,0));await page.getByRole('button',{name:'从媒体库插入正文图片'}).click();await page.getByRole('button',{name:'测试图片'}).click();await page.getByLabel('图片替代文本',{exact:true}).fill('a[b]');await page.getByRole('button',{name:'使用图片',exact:true}).click();assert.ok((await page.getByRole('textbox',{name:'Markdown 正文'}).inputValue()).startsWith('![a\\[b\\]]'));
