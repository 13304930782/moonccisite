...
for file in "${backend_files[@]}"; do
  [[ "$file" =~ ^(src/(index.js|routes/analytics.js|lib/(socialConfig|socialProviders).js)|database/migrations/202609140001_analytics.sql)$ ]]
  test -s "server/$file"
...
NODE
log '仅应用新增访问统计表，保留历史 SQL 校验记录……'
node server/scripts/migrate-analytics.js "$live"
log '更新统计及微软登录 API，保留 Google CF 代理与环境配置……'
server_changed=1
for file in "${backend_files[@]}"; do
  install -d -o mooncci -g mooncci -m 755 "$(dirname "$live/$file")"
  install -o mooncci -g mooncci -m 644 "server/$file" "$live/$file"
done
sed -n 's|  server/|  |p' SHA256SUMS | grep -v '  scripts/migrate-analytics.js$' > "$backup/backend-checksums"
