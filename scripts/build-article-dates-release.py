...
files=['src/routes/posts.js','src/routes/articleDrafts.js']
baseline=json.loads((root/'scripts/article-dates-baseline.json').read_text())
for n in files:
...
entries['deploy.sh']=(root/'scripts/deploy-article-dates.sh').read_text(encoding='utf-8-sig').replace('\r\n','\n').encode()
...
out=root/'.cache'/f"mooncci-article-dates-{release['revision'][:12]}.tar.gz"
...
result={**release,'package':str(out),'bytes':out.stat().st_size,'sha256':sha,'scope':'article-dates-no-migrations'}
(root/'.cache/article-dates-release.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8',newline='\n');print(json.dumps(result,indent=2))
