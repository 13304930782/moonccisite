...
entries['deploy.sh'] = (root / 'scripts/deploy-analytics.sh').read_bytes().replace(b'\r\n', b'\n')
files = ['src/index.js', 'src/routes/analytics.js', 'src/lib/socialConfig.js', 'src/lib/socialProviders.js', 'database/migrations/202609140001_analytics.sql']
for name in files:
    entries['server/' + name] = (root / 'server' / name).read_bytes().replace(b'\r\n', b'\n')
entries['server/scripts/migrate-analytics.js'] = (root / 'server/scripts/migrate-analytics.js').read_bytes().replace(b'\r\n', b'\n')
...
output = root / '.cache' / f'mooncci-analytics-{revision[:12]}.tar.gz'
...
result = {'revision': revision, 'package': str(output), 'bytes': output.stat().st_size, 'sha256': digest, 'scope': 'analytics-microsoft-with-additive-migration'}
