...
entries['deploy.sh'] = (root / 'scripts/deploy-google-login.sh').read_bytes().replace(b'\r\n', b'\n')
files = ['src/routes/socialLogin.js', 'src/lib/socialConfig.js', 'src/lib/socialProviders.js']
for name in files:
...
result = {'revision': revision, 'package': str(output), 'bytes': output.stat().st_size, 'sha256': digest, 'scope': 'google-redirect-login'}
