...
for file in "${backend_files[@]}"; do
  [[ "$file" =~ ^(src/(index.js|routes/(account|admin|socialLogin|posts|comments).js|lib/accountSettings.js)|database/migrations/202609100002_account_settings.sql)$ ]]
  test -s "server/$file"
...
NODE
log '仅应用新增账号设置表，保留历史 SQL 校验记录……'
node server/scripts/migrate-account-settings.js "$live"
log '更新账号 API，保留 Google CF 代理与环境配置……'
server_changed=1
for file in "${backend_files[@]}"; do
  install -d -o mooncci -g mooncci -m 755 "$(dirname "$live/$file")"
  install -o mooncci -g mooncci -m 644 "server/$file" "$live/$file"
done
sed -n 's|  server/|  |p' SHA256SUMS | grep -v '  scripts/migrate-account-settings.js$' > "$backup/backend-checksums"
